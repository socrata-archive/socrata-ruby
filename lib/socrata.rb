...
class Socrata
  ...
  attr_reader :error, :config, :batching, :batch_queue

  default_options[:headers] = {'Content-type' => 'application/json'}
  format :json

  DEFAULT_CONFIG = {
    :base_uri => "http://opendata.socrata.com/api"
  }
  ...
  def user(uid)
    return User.new(get_request("/users/#{uid}.json"), @party)
  end
  ...
  def create(name, description = "", tags = [], public = true)
    ...
    data_hash[:flags] = ['dataPublicRead'] if public
